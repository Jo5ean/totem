...
// Datos de sectores del CSV proporcionado
const SECTORES_CSV = [
  { id_sector: 1, sector: '0', codare: '', nombre: '-.-' },
  { id_sector: 2, sector: '1', codare: '11', nombre: 'ARTES Y CIENCIAS' },
  { id_sector: 3, sector: '2', codare: '12', nombre: 'ECONOMÍA Y ADMINISTRACIÓN' },
  { id_sector: 4, sector: '3', codare: '13', nombre: 'CIENCIAS JURÍDICAS' },
  { id_sector: 5, sector: '4', codare: '14', nombre: 'INGENIERÍA' },
  { id_sector: 6, sector: '5', codare: '15', nombre: 'ARQUITECTURA Y URBANISMO' },
  { id_sector: 7, sector: '6', codare: '16', nombre: 'ESCUELA UNIVERSITARIA DE TRABAJO SOCIAL' },
  { id_sector: 8, sector: '7', codare: '17', nombre: 'ESCUELA UNIVERSITARIA DE EDUCACIÓN FÍSICA' },
  { id_sector: 9, sector: '8', codare: '18', nombre: 'ESCUELA UNIVERSITARIA DE TURISMO' },
  { id_sector: 10, sector: '9', codare: '19', nombre: 'CIENCIAS AGRARIAS Y VETERINARIAS' }
];

async function mapearCarrerasAutomatico() {
  ...
  try {
    ...
    // 4. Reporte final
    console.log('\n📋 REPORTE FINAL:');
    console.log(`✅ Mapeos realizados: ${mapeosRealizados}`);
    console.log(`❌ Mapeos no encontrados: ${mapeosNoEncontrados.length}`);
    
    if (mapeosNoEncontrados.length > 0) {
      console.log('\n📝 CARRERAS NO MAPEADAS:');
      mapeosNoEncontrados.forEach(item => {
        console.log(`  - Código ${item.codigo}: ${item.nombre} (${item.razon})`);
      });
    }

    // 5. Verificar el resultado
    const estadisticasFinales = await obtenerEstadisticasMapeados();
    console.log('\n📊 ESTADÍSTICAS FINALES:');
    console.log(`  - Total carreras TOTEM: ${estadisticasFinales.total}`);
    console.log(`  - Mapeadas: ${estadisticasFinales.mapeadas}`);
    console.log(`  - No mapeadas: ${estadisticasFinales.noMapeadas}`);
    console.log(`  - Porcentaje mapeado: ${estadisticasFinales.porcentaje}%`);
...
async function obtenerEstadisticasMapeados() {
  ...
  return { total, mapeadas, noMapeadas, porcentaje };
}

// Ejecutar el script
mapearCarrerasAutomatico()
  .then(() => {
